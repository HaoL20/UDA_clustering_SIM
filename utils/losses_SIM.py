...
class feat_reg_ST_loss(nn.Module):
    ...
    def dist(self, tensor):
        if isinstance(self.dist_func, int):
            return torch.norm(tensor,
                              p=self.dist_func) / tensor.numel()  # sum(abs(tensor)**p)**(1./p) , default output is scalar
        else:
    ...
    def feature_processing(self, feat, softmax, domain, argmax_dws_type='bilinear'):
        ...
        if argmax_dws_type == 'nearest': argmax_dws = torch.squeeze(
            F.interpolate(torch.unsqueeze(argmax.float(), dim=1), size=(h, w), mode='nearest'), dim=1)  # size B x h x w
        ...
        argmax_dws = argmax_dws.view(-1)  # size N (N = B x h x w)
        ...
        if domain == 'source':
            self.source_feat = feat  # N x F, feat of each pixel
            self.source_argmax = argmax_dws  # N, max probability of each pixel (downsampling)
            self.source_argmax_full = torch.max(softmax, dim=1)[
                1]  # B x H x W, max probability of each pixel (ori size)
            self.source_softmax = softmax_dws  # N x C, probability of each class on the pixel (downsampling)
            self.source_softmax_full = softmax  # B x C x H x W, probability of each class on the pixel (ori size)
        else:
    ...
    def compute_centroids_mixed(self, centroids_smoothing=-1):
        ...
        feat_list_source_BG, feat_list_source_FG = [], []
        feat_list_target_BG, feat_list_target_FG = [], []
        centroid_list_source_BG, centroid_list_target_BG = [], []
        centroid_list_source_FG, FG_centroid_list_target_FG = [], []

        # compute stuff centroid
        for i in range(len(self.BG_LABEL)):
            # boolean tensor, True where features belong to class i
            source_mask = torch.eq(self.source_argmax.detach(), i)  # size Ns_BG
            target_mask = torch.eq(self.target_argmax.detach(), i)  # size Nt_
            ...
            if source_feat_i.size(0) > 0:
                feat_list_source_BG.append(source_feat_i)
                centroid = torch.mean(source_feat_i, dim=0, keepdim=True)  # size 1 x F
                centroid_list_source_BG.append(centroid)
            else:
                feat_list_source_BG.append(None)
                centroid_list_source_BG.append(
                    torch.tensor([[float("Inf")] * self.source_feat.size(1)], dtype=torch.float).to(
                        self.device))  # size 1 x F

            if target_feat_i.size(0) > 0:
                feat_list_target_BG.append(target_feat_i)
                centroid = torch.mean(target_feat_i, dim=0, keepdim=True)  # size 1 x F
                centroid_list_target_BG.append(centroid)
            else:
                feat_list_target_BG.append(None)
                centroid_list_target_BG.append(
                    torch.tensor([[float("Inf")] * self.target_feat_i.size(1)], dtype=torch.float).to(
                        self.device))  # size 1 x F

        # compute instances centroid and instances feat pools
        for i in range(len(self.FG_LABEL)):
            ...
            target_mask_np = target_mask.cpu().numpy().astype(int)
            # compute all instances(connected component) mask, and area
            target_instances_mask_np, target_instances_mask_area = sklabel(target_mask_np, background=0,
                                                                           return_num=True, connectivity=2)
            target_instances_mask = torch.LongTensor(target_instances_mask_np).cuda()
            target_instances_mask_area = np.zeros(target_instances_mask_area, dtype=int)
            ...
            if source_feat_i.size(0) > 0:
                feat_list_source_BG.append(source_feat_i)
                centroid = torch.mean(source_feat_i, dim=0, keepdim=True)  # size 1 x F
                BG_centroid_list.append(centroid)
            else:
                feat_list_source_BG.append(None)
                ...
                BG_centroid_list.append(torch.tensor([[float("Inf")] * self.source_feat.size(1)], dtype=torch.float).to(
                    self.device))  # size 1 x F
        ...
        if centroids_smoothing >= 0.:
            ...
            self.mixed_centroids_avg = torch.where(self.mixed_centroids_avg != float('inf'), self.mixed_centroids_avg,
                                                   self.mixed_centroids)
            ...
            self.mixed_centroids = torch.where(self.mixed_centroids == float('inf'), self.mixed_centroids_avg.detach(),
                                               self.mixed_centroids)
            self.mixed_centroids = centroids_smoothing * self.mixed_centroids + (
                    1 - centroids_smoothing) * self.mixed_centroids_avg.detach()
    ...
    def intra_domain_c2c(self):
        ...
        for i in indices:
            ...
            c_dist = c_dist + torch.mean(
                self.dist(centroids[i, :] - centroids[indices_but_i, :]))  # distance among intra-domain clusters
    ...
    def similarity_dsb(self, feat_domain, temperature=1.):
        ...
        seen_classes = [i for i in range(self.num_class) if
                        not torch.isnan(centroids[i, 0]) and not centroids[i, 0] == float(
                            'Inf')]  # list of C elems, True for seen classes, False elsewhere
    ...
    def forward(self, **kwargs):
        ...
        if 'source_gt' in kwargs.keys():
            ...
            self.source_argmax = torch.squeeze(
                F.interpolate(torch.unsqueeze(self.source_argmax_full.float(), dim=1), size=(self.hs, self.ws),
                              mode='nearest'), dim=1).view(-1)
        ...
        output = {'c_dist': c_dist, 'f_dist_source': f_dist_source, 'f_dist_target': f_dist_target,
                  'ortho_loss': ortho_loss, 'sparse_loss': sparse_loss}
        return output
