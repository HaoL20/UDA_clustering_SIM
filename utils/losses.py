...
class feat_reg_ST_loss(nn.Module):
    ...
    def feature_processing(self, feat, softmax, domain, argmax_dws_type='bilinear'):
        ...
        if domain == 'source':
            ...
            self.source_argmax_full = torch.max(softmax,dim=1)[1]  # B x H x W, max probability of each pixel (ori size)
...
if __name__ == '__main__':
    f = feat_reg_ST_loss()
    torch.manual_seed(1)
    feature_s = torch.randint(0, 19, (2, 3, 2, 2), dtype=torch.float).cuda()
    feature_t = torch.randint(0, 19, (2, 3, 2, 2), dtype=torch.float).cuda()
    feat_s_softmax = torch.nn.functional.softmax(torch.rand((2, 18, 4, 4)), dim=1).cuda()
    feat_t_softmax = torch.nn.functional.softmax(torch.rand((2, 18, 4, 4)), dim=1).cuda()

    _, argmax_s = torch.max(feat_s_softmax, dim=1)  # size B x H x W
    _, argmax_t = torch.max(feat_t_softmax, dim=1)  # size B x H x W
    gt_s = torch.squeeze(F.interpolate(torch.unsqueeze(argmax_s.float(), dim=1), size=(2, 2), mode='nearest'), dim=1)  # size B x h x w
    gt_t = torch.squeeze(F.interpolate(torch.unsqueeze(argmax_t.float(), dim=1), size=(2, 2), mode='nearest'), dim=1)  # size B x h x w


    f.feature_processing(feature_s, feat_s_softmax, domain='source')
    f.feature_processing(feature_t, feat_t_softmax, domain='target')
