...
def get_files(dir):
    ...
    for root, dirs, files in os.walk(dir):
        for filename in files:
            if 'labelIds' in filename:
                fileslist.append(os.path.join(root, filename))
    return sorted(fileslist)


def compute_mIoU(gt_dir, pred_dir, devkit_dir='datasets/Cityscapes', silence=False):
    """
    ...
    gt_imgs = get_files(gt_dir)
    pred_imgs = get_files(pred_dir)
