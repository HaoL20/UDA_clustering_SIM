...
from datasets.cityscapes_Dataset import City_DataLoader, inv_preprocess, decode_labels, colorize_mask
        ...
        with torch.no_grad():
            ...
            for x, y, id_gt in tqdm_batch:
                ...
                if args.save_prediction_to is not None:
                    ...
                    id_gt = id_gt[0].split('/')[-1]
                    id_mask_dir = os.path.dirname(os.path.join(args.save_prediction_to + '/id_mask/' + id_gt))
                    if not os.path.exists(id_mask_dir):
                        os.makedirs(id_mask_dir)
                    color_mask_dir = os.path.dirname(os.path.join(args.save_prediction_to + '/color_mask/' + id_gt))
                    if not os.path.exists(color_mask_dir):
                        os.makedirs(color_mask_dir)
                    output.save(args.save_prediction_to + '/id_mask/' + id_gt)
                    output_col.save(args.save_prediction_to + '/color_mask/' + id_gt)
